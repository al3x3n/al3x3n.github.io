...
jQuery( document ).ready( function( $ ){
	...
	var lsweb = {
		...
		init: function(){
			...
			// Purchase code empty input check
			$( '#privacy-download-information' ).on( 'submit', function(e){
				if( $( '#privacy-download-information input' ).first().val().length < 30 ){
					e.preventDefault();
					alert( 'Your purchase code does not appear to be valid.' );
				}
			});
		},
		...
		setNavigation: function(){
				...
				TweenMax.fromTo( $( 'header' )[0], 1, {
					...
					onComplete: function(){
						...
						/*kmUI.textUnderline.init({
							marginTop: -10,
							selector: '.km-ui-text-underline > a:not(.button, .no-underline)'
						});*/
					}
				});
			...
			var menuUnderlineTimeout;

			jQuery(window).resize(function() {

				clearTimeout( menuUnderlineTimeout );
				/*menuUnderlineTimeout = setTimeout(function() {

					kmUI.textUnderline.set( jQuery('nav.km-ui-text-underline a.active') );
				}, 100);*/
			});
		},
